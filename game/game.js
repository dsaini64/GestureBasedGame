...
socket.on('information', function (data)
{
    for(var i = 0; i < 5; i++)
    {
        blueteam[i].position = new PIXI.Point(data.blueteam[i].position.x,data.blueteam[i].position.y);
        orangeteam[i].position = new PIXI.Point(data.orangeteam[i].position.x,data.orangeteam[i].position.y);
    }
});
